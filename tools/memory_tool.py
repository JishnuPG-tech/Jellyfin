...
def _get_db_conn():
    """Return a SQLite connection to the state database with thread tuning."""
...
def _init_db():
    """Initialize the cognitive_memories table in the database if not present."""
    conn = _get_db_conn()
    try:
        conn.execute("""
        ...
        logger.error("Failed to initialize cognitive_memories table: %s", exc)
    finally:
        conn.close()
...
class MemoryStore:
    ...
    def load_from_disk(self):
        ...
        try:
            ...
            cursor = conn.cursor()
            cursor.execute(
                "SELECT category, content FROM cognitive_memories ORDER BY created_at ASC"
            )
            ...
            for category, content in rows:
                if category == "user":
                    db_user_entries.append(content)
                else:
                    db_memory_entries.append(content)
            
            if db_user_entries or db_memory_entries:
                self.memory_entries = db_memory_entries
                self.user_entries = db_user_entries
                db_loaded = True
        except Exception as exc:
            logger.error("Failed to load memories from SQLite: %s", exc)
        finally:
            if conn:
                conn.close()
        ...
        if not db_loaded:
            ...
            # Auto-migrate Markdown files into SQLite database
            if self.memory_entries or self.user_entries:
                try:
                    conn = _get_db_conn()
                    now = time.time()
                    for content in self.memory_entries:
                        conn.execute(
                            "INSERT OR IGNORE INTO cognitive_memories (category, content, created_at) VALUES (?, ?, ?)",
                            ("memory", content, now)
                        )
                    for content in self.user_entries:
                        conn.execute(
                            "INSERT OR IGNORE INTO cognitive_memories (category, content, created_at) VALUES (?, ?, ?)",
                            ("user", content, now)
                        )
                    ...
                    logger.error("Failed to backfill Markdown memories into SQLite: %s", exc)
                finally:
                    if conn:
                        conn.close()
        else:
            # Sync cache Markdown files on disk so they stay updated
            try:
                self._write_file(mem_dir / "MEMORY.md", self.memory_entries)
                self._write_file(mem_dir / "USER.md", self.user_entries)
            except Exception as exc:
                logger.error("Failed to write SQLite memories to Markdown cache: %s", exc)
    ...
    def save_to_disk(self, target: str):
        """Persist entries to both SQLite database and Markdown files."""
        # 1. Save to Markdown file for backwards-compatibility hybrid storage
        ...
        try:
            conn = _get_db_conn()
            now = time.time()
            conn.execute("DELETE FROM cognitive_memories WHERE category = ?", (target,))
            for content in self._entries_for(target):
                conn.execute(
                    "INSERT OR IGNORE INTO cognitive_memories (category, content, created_at) VALUES (?, ?, ?)",
                    (target, content, now)
                )
            ...
            logger.error("Failed to save memories to SQLite: %s", exc)
        finally:
            if conn:
                conn.close()
