...
7. **NFS SQLite Lock Safety**:
   ...
   - Schema inspection always copies `jellyfin.db` to `/tmp/check_jellyfin.db` first, so the check is fully lock-free.
...
8. **Jellyfin Schema Compatibility Guard**:
   - **Root cause addressed**: Jellyfin 10.9.11 requires `Users.MaxParentalAgeRating`. Databases created or upgraded by Jellyfin 10.10+/10.11+ use `MaxParentalRatingScore` instead, causing a fatal `no such column: u.MaxParentalAgeRating` startup crash.
   - **Startup preflight**: Before Jellyfin launches, `entrypoint.sh` inspects a lock-free copy of the database schema using `pragma_table_info('Users')`. If `MaxParentalAgeRating` is absent from an initialized `Users` table, the schema is declared incompatible.
   - **Surgical state reset**: Only Jellyfin's database and migration files are removed (`jellyfin.db*`, `migrations.xml`, `system.xml`, `network.xml`). The virtual media library (`/data/jellyfin/media/`) and all Apex state (`/data/apex/`) are **fully preserved**.
   - **Crash-loop recovery**: When Jellyfin exits inside the supervisor loop, the schema check runs again against the current database state *before* deciding to restart. If incompatible, the state reset executes (once); if the schema is still broken after reset, the restart loop halts cleanly rather than thrashing.
   - **Diagnostic backup**: Every quarantine writes a timestamped snapshot to `/data/jellyfin/backups/incompatible_schema_<timestamp>/` for forensic inspection.
