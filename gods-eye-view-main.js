...
async function init() {
  ...
  try {
    ...
    // Hide Cesium's default globe if Google 3D Tiles are loaded
    ...
    loaderStatus.textContent = 'Loading terrain & 3D tiles...';
    let tileset = null;
    if (googleApiKey) {
      try {
      ...
      console.warn('[Init] GOOGLE_MAPS_API_KEY not set, using keyless Cesium/OSM globe.');
      viewer.scene.globe.show = true;
    }
